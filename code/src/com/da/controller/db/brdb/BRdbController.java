package com.da.controller.db.brdb;
...
public class BRdbController extends BaseController {
	...
	public ModelAndView listAllTable() throws Exception{
		...
		mv.setViewName("db/brdb/table_list");
	...
	public Object backupAll(){
		...
		String kackupPath;
		try {
			kackupPath = DbDA.getDbDA().backup("").toString();		//调用数据库备份
	...
	public Object backupTable(){
		...
		Map<String,Object> map = new HashMap<String,Object>();
		pd = this.getPageData();
		String TABLENAME = pd.getString("datable");					//页面ajax传过来的表名
		List<PageData> pdList = new ArrayList<PageData>();
		String kackupPath;
		try {
			kackupPath = DbDA.getDbDA().backup(TABLENAME).toString();	//调用数据库备份
			if(Tools.notEmpty(kackupPath) && !"errer".equals(kackupPath)){
				pd.put("DB_ID", this.get32UUID());				//主键
				pd.put("USERNAME", username);						//操作用户
				pd.put("BACKUP_TIME", Tools.date2Str(new Date()));	//备份时间
	...
	public Object dbRecover(){
		...
		try {
			String returnStr = DbDA.getDbDA().recover(TABLENAME, SQLPATH).toString();
	...
	public ModelAndView list(Page page) throws Exception{
		...
		Map<String,String> DBmap = DbDA.getDBParameter();
		mv.setViewName("db/brdb/brdb_list");
	...
	public ModelAndView goEdit()throws Exception{
		...
		mv.setViewName("db/brdb/brdb_edit");
