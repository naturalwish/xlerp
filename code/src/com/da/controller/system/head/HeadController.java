package com.da.controller.system.head;
...
import com.da.service.system.userphoto.UserPhotoManager;
import com.da.util.AppUtil;
import com.da.util.Const;
import com.da.util.Jurisdiction;
import com.da.util.PageData;
import com.da.util.SmsUtil;
import com.da.util.Tools;
import com.da.util.Watermark;
import com.da.util.mail.SimpleMailSender;
...
public class HeadController extends BaseController {
	...
	public Object getList() {
		...
		try {
			...
			if(null == pds){
				...
				pd.put("USER_ID", Jurisdiction.getUserID());//当前登录者用户ID
				pds = userService.findById(pd);
				if(pds==null){//系统用户查不到的话 根据 会员用户再查
					pds = appuserService.findByUiId(pd);
				}
	...
	public Object sendEmail(){
		...
		if(null != strEMAIL && !"".equals(strEMAIL)){
			String strEM[] = strEMAIL.split(",da,");
			if(strEM.length == 4){
				if("yes".endsWith(isAll)){
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception{
		...
		if(null != strFWATERM && !"".equals(strFWATERM)){
			String strFW[] = strFWATERM.split(",da,");
			if(strFW.length == 5){
		...
		if(null != strIWATERM && !"".equals(strIWATERM)){
			String strIW[] = strIWATERM.split(",da,");
			if(strIW.length == 4){
		...
		if(null != strWEBSOCKET && !"".equals(strWEBSOCKET)){
			String strIW[] = strWEBSOCKET.split(",da,");
			if(strIW.length == 5){
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",da,"+pd.getString("PORT")+",da,"+pd.getString("EMAIL")+",da,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",da,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",da,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",da,"+pd.getString("fcontent")+",da,"+pd.getString("fontSize")+",da,"+pd.getString("fontX")+",da,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",da,"+pd.getString("imgUrl")+",da,"+pd.getString("imgX")+",da,"+pd.getString("imgY"));	//图片水印配置
		Watermark.fushValue();
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	...
	public ModelAndView saveSys3() throws Exception{
		...
		Tools.writeFile(Const.WEBSOCKET,pd.getString("WIMIP")+",da,"+pd.getString("WIMPORT")+",da,"+pd.getString("OLIP")+",da,"+pd.getString("OLPORT")+",da,"+pd.getString("FHsmsSound"));	//websocket配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView saveSys4() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.LOGINEDIT,pd.getString("isZhuce")+",da,"+pd.getString("isMusic"));	//登录页面配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
