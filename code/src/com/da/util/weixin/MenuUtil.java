...
import com.da.entity.weixin.Button;
import com.da.entity.weixin.CommonButton;
import com.da.entity.weixin.ComplexButton;
import com.da.entity.weixin.Menu;
import com.da.entity.weixin.ViewButton;
		...
		if(null != M2.getString("TITLE") && !"".equals(M2.getString("TITLE").trim())){
			...
			for(int i=0;i<listm2.size();i++){
				...
				if("click".equals(pd.getString("TYPE"))){
					...
					//动情固定第2个为商城时调用小程序,此处特殊处理,绕行不要被坑
					if(cb.getName().indexOf("商城")!=-1){
						System.out.println("-----------------Name1:"+cb.getName());
						cb.setType("miniprogram");
						cb.setKey("");
						cb.setUrl("http://www.baidu.com");
						cb.setAppid("wx9b2c7cd55e9cb5ab");
						cb.setPagepath("pages/index/index");
					}
					arrayB[i] = cb;
				}else{
					ViewButton vb = new ViewButton();
