...
<body class="no-skin">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<c:if test="${dbtype != 'sqlserver' }">
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' id='daid${vs.index}' value="${var}" class="ace" /><span class="lbl"></span></label>
											</td>
											</c:if>
	...
	<script type="text/javascript">
		...
		//备份单表
		function backupTable(datable){
			bootbox.confirm("确定要备份这表[ "+datable+" ]吗?", function(result) {
				if(result) {
					backupAjax(datable,"backupts");
				}
			});
		}
				...
				if(result) {
					...
					for(var i=0;i < document.getElementsByName('ids').length;i++){
					  if(document.getElementsByName('ids')[i].checked){
					  	...
					  	else str += ',da,' + document.getElementsByName('ids')[i].value;
					  	
					  	if(fid=='') fid += document.getElementsByName('ids')[i].id;
					  	else fid += ',da,' + document.getElementsByName('ids')[i].id;
					  }
					}
					if(str==''){
						bootbox.dialog({
						...
						if(msg == '确定要批量备份这些表吗？'){
							var arrTable = str.split(',da,');
							var arrFid = fid.split(',da,');
							for(var dai=0;dai<arrTable.length;dai++){
								backupAjax(arrTable[dai],arrFid[dai]);
							}
						}
					}
				}
