...
<body class="no-skin">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
							<thead>
								<tr>
									...
									<th class="center"><i class="ace-icon fa fa-phone bigger-110 hidden-480"></i>手机号</th>
								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										<tr>
											...
											<td class="center">${user.START_TIME}</td>
											...
											<td class='center' style="height: 20px;">
												<label>
													<input name="switch-field-1"  onclick="updateSTATUS('${user.USER_ID}',this.checked)" class="ace ace-switch ace-switch-3" type="checkbox" <c:if test="${user.PROXYSTATE == 1 }">checked="checked" </c:if> >
													<span class="lbl"></span>
												</label>
											</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
														<a class="btn btn-xs btn-info" onclick="userAddress('${user.USER_ID }');">收货地址</a>
														<a class="btn btn-xs btn-success" onclick="userCoupon('${user.USER_ID }');">会员优惠券</a>
													</c:if>
													<c:if test="${QX.edit == 1 }">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
        ...
        //更新商品是否上架
        function updateSTATUS(user_ID,c){
            var val = c? 1:0;
            $.ajax({
                type: "POST",
                url: "<%=basePath%>happuser/editU.do?tm="+new Date().getTime(),
                data: {USER_ID:user_ID,PROXYSTATE:val},
                dataType:'json',
                cache: false,
		...
		function userCoupon(userID) {
			...
			diag.Drag = true;
			diag.Title = "会员优惠券";
			diag.URL = '<%=basePath%>usercoupon/list.do?USER_ID='+userID;
			diag.Width = 1200;
			diag.Height = 650;
		...
		function userAddress(userID) {
			...
			diag.Drag = true;
			diag.Title = "收货地址";
			diag.URL = '<%=basePath%>address/list.do?USER_ID='+userID;
			diag.Width = 1200;
			diag.Height = 650;
			diag.Modal = true;				//有无遮罩窗口
			diag.ShowMaxButton = true;	//最大化按钮
