...
<link rel="stylesheet" href="static/login/css/lrtk.css" />
...
<!-- 软键盘控件end -->
 <style type="text/css">
  ...
  <script>
  		//window.setTimeout(showda,3000);
  		var timer;
		function showda(){
			dai = 1;
			//关闭提示晃动屏幕，注释掉这句话即可
			timer = setInterval(xzda2, 10);
		};
		var current = 0;
		function xzda(){
			current = (current)%360;
...
<body>
	...
	<a target="_blank"  href="http://wpa.qq.com/msgrd?v=3&uin=12345678&site=qq&menu=yes" id="udesk-feedback-tab" class="udesk-feedback-tab-left" style="display: block; background-color: black;"></a>
	<!-- 代码 结束 -->--%>
	<c:if test="${pd.isMusic == 'yes' }">
	<div style="display: none">
	    <audio src="static/login/music/da1.mp3" autoplay=""></audio>
	</div>	
	</c:if>
	...
	<div style="width:100%;text-align: center;margin: 0 auto;position: absolute;">
		...
		<div id="loginbox" >
			...
			<div class="controls">
				<div class="main_input_box">
					<font color="white"><span id="nameerr">Copyright © DA Admin 2018-2020</span></font>
				</div>
			</div>
		</div>
		</div>
	...
	<script type="text/javascript">
		...
		function severCheck(){
			if(check()){
				...
				var code = "qq2008da"+loginname+",da,"+password+"QQ2012da"+",da,"+$("#code").val();
				$.ajax({
					type: "POST",
					url: 'login_login',
	...
	function register(){
		if(rcheck()){
			...
			$.ajax({
		    	...
		    	data: {USERNAME:$("#USERNAME").val(),PASSWORD:$("#PASSWORD").val(),NAME:$("#name").val(),EMAIL:$("#EMAIL").val(),rcode:$("#rcode").val(),FKEY:$.md5('USERNAME'+nowtime+',da,'),tm:new Date().getTime()},
				dataType:'json',
				cache: false,
				success: function(data){
					if("00" == data.result){
