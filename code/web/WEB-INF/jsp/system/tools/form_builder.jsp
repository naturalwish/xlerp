...
<base href="<%=basePath%>">
	...
	<!-- 表单构建组建 -->
	<link rel="shortcut icon" href="favicon.ico"> <link href="plugins/daform/css/bootstrap.min.css?v=3.3.5" rel="stylesheet">
    <link href="plugins/daform/css/animate.min.css" rel="stylesheet">
    <link href="plugins/daform/css/style.min.css?v=4.0.0" rel="stylesheet"><base target="_blank">
	<!-- 日期框 -->
...
<body class="gray-bg">
	...
	<!-- 表单构建组建 -->
    <script src="plugins/daform/js/bootstrap.min.js?v=3.3.5"></script>
    <script src="plugins/daform/js/content.min.js?v=1.0.0"></script>
    <script src="plugins/daform/js/jquery-ui-1.10.4.min.js"></script>
    <script src="plugins/daform/js/beautifyhtml/beautifyhtml.js"></script>
	<!-- 百度富文本编辑框-->
	...
	function getUeditorFormHtml(html,msg,isgx){
		var arryUe = html.split('<div class="ueQ2012Que"></div>');
		if(arryUe.length == 3){
			var uejscode = "<script id=\"editor\" type=\"text/plain\" style=\"width:96%;height:200px;\"><\/script>";
			if(msg == '1'){
				if(isgx == '2'){
					ueditorHtml = arryUe[1];
					return arryUe[0] + '<div class="ueQ2012Que"></div>' + uejscode + '<div class="ueQ2012Que"></div>' + arryUe[2];
				}else{
					return arryUe[0] + '<div class="ueQ2012Que"></div>' + ueditorHtml + '<div class="ueQ2012Que"></div>' + arryUe[2];
				}
			}else{
	...
	function getSelectFormHtml(html,msg,isgx){
		var arrySe = html.split('<div class="selQ2012Qsel"></div>');
		if(arrySe.length == 3){
			...
			if(msg == '1'){
				if(isgx == '2'){
					return arrySe[0] + '<div class="selQ2012Qsel"></div>' + selectcode + '<div class="selQ2012Qsel"></div>' + arrySe[2];
				}else{
					selectHtml = arrySe[1];
					return html;
				}
	...
	function getFileFormHtml(html,msg,isgx){
		var arryFile = html.split('<div class="fileQ2012Qfile"></div>');
		if(arryFile.length == 3){
			var filecode = "<input type=\"file\" id=\"tp\" name=\"tp\" />";
			if(msg == '1'){
				if(isgx == '2'){
					fileHtml = arryFile[1];
					return arryFile[0] + '<div class="fileQ2012Qfile"></div>' + filecode + '<div class="fileQ2012Qfile"></div>' + arryFile[2];
				}else{
					return arryFile[0] + '<div class="fileQ2012Qfile"></div>' + fileHtml + '<div class="fileQ2012Qfile"></div>' + arryFile[2];
				};
			}else{
