...
$(document).ready(function(){
	...
	$("#uploadify1").uploadify({
		...
		'buttonText':	"files",
		'scriptData':	{'uploadPath':'/uploadFiles/uploadImgs/'},//这个参数用于传递用户自己的参数，此时'method' 必须设置为GET, 后台可以用request.getParameter('name')获取名字的值
		'method'	:	"GET",
	...
	$.ajax({
		...
		success: function(data){
			 ...
			 if("success" == data.result){
				 ...
				 $("#encoderImgId").attr("src",locat+'/uploadFiles/uploadImgs/' + data.encoderImgId);
			 }else{
				 $("#encoderContent").tips({
						side:3,
			            msg:'生成失败,后台有误',
				 ...
				 $("#encoderImgId").attr("src",locat+"/uploadFiles/uploadImgs/default.png");
				 return;
			 }
